...
func QueryTag(c *fiber.Ctx, db *gorm.DB) error {
	...
	response := ResponseFactory(c, fiber.StatusOK, "Tag資料查詢成功", &responseData)
	return c.Status(fiber.StatusOK).JSON(response)
}

func CreateTag(c *fiber.Ctx, db *gorm.DB) error {
	var clientData TagCreateRequest
	...
	response := ResponseFactory[any](c, fiber.StatusOK, "成功建立Tag", nil)
	return c.Status(fiber.StatusOK).JSON(response)
}
